...
# Function to set variables based on the retrieved customer data and return as a list
def set_variables_from_data(customer_data):
    if isinstance(customer_data, list) and len(customer_data) > 0 and isinstance(customer_data[0], dict):
        data = customer_data[0]  # Assume we're only working with the first dictionary in the list

        # Set variables from the dictionary values, converting each to a string
        Ingredients = str(data.get("Ingredients", ""))
        allergy = str(data.get("allergy", ""))
        budget = str(data.get("budget", ""))
        calories = str(data.get("calories", ""))
        customerid = str(data.get("customerid", ""))
        dislikes = str(data.get("dislikes", ""))
        location = str(data.get("location", ""))
        preferences = str(data.get("preferences", ""))

        # Return the variables as a list (if the assistant accepts a list)
        return [
            Ingredients,
            allergy,
            budget,
